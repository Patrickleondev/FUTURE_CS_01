...
> ⚠️ **Ethical Note:** This assessment strictly adhered to a read-only, non-destructive approach. No data was accessed, modified, or exfiltrated.
...
```bash
whatweb -v http://demo.testfire.net | tee evidence/whatweb_results.txt
```
...
```bash
nikto -h http://demo.testfire.net -o evidence/nikto_results.txt
```
